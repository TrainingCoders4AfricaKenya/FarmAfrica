...
class NotificationsController extends Controller {
        ...
        if (isset($_POST['RNotifications'])) {
            ...
            try {
                ...
                if ($saveOK) {
                    ...
                    $this->redirect(array('view', 'id' => $model->notificationID));
                } else {
                ...
                Utils::log('EXCEPTION', 'EActiveResourceRequestException ON CREATE RNotifications | CODE: '
                ...
                if ($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])) {
                    ...
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO CREATE RNotifications | '
                ...
                //error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO CREATE RNotifications  | '
                        . CJSON::encode($exc), __CLASS__, __FUNCTION__, __LINE__);
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 'sorryAnErrorOccurredWhileCreatingThe{model}Record', array('{model}' => 'Notifications')));
                $this->render('create', array(
                    'model' => $model,
                ));
                Yii::app()->end();
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['RNotifications'])) {
            ...
            try {
                ...
                //Catch EActiveResourceRequestException which is thrown in case of any errors
                Utils::log('EXCEPTION', 'EActiveResourceRequestException ON UPDATE RNotifications | CODE: '
                        . $resourceExc->getCode() . ' | MESSAGE: ' . $resourceExc->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
                //an error occurred while doing processing
                $modelErrors = array();
                $errorMsg = $resourceExc->getMessage();
                ...
                if ($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])) {
                    //error that occurred wasn't model-related
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO UPDATE RNotifications | '
                            . CJSON::encode($errorMsg), __CLASS__, __FUNCTION__, __LINE__);
                    Yii::app()->user->setFlash('error', 'Error occurred');
                } else {
                    $model->addErrors($modelErrors['modelErrors']);
                ...
                //error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO UPDATE RNotifications  | '
                        . CJSON::encode($exc), __CLASS__, __FUNCTION__, __LINE__);
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 'sorryAnErrorOccurredWhileUpdatingThe{model}Record', array('{model}' => 'Notifications')));
                $this->render('update', array(
                    'model' => $model,
                ));
                Yii::app()->end();
    ...
    public function actionDelete($id) {
        ...
        if (isset($_POST['RNotifications'])) {
            ...
            try {
                ...
                //Catch EActiveResourceRequestException which is thrown in case of any errors
                Utils::log('EXCEPTION', 'EActiveResourceRequestException ON DELETE RNotifications | CODE: '
                        . $resourceExc->getCode() . ' | MESSAGE: ' . $resourceExc->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
                //an error occurred while doing processing
                $modelErrors = array();
                $errorMsg = $resourceExc->getMessage();
                ...
                if ($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])) {
                    //error that occurred wasn't model-related
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO DELETE RNotifications | '
                            . CJSON::encode($errorMsg), __CLASS__, __FUNCTION__, __LINE__);
                    Yii::app()->user->setFlash('error', 'Error occurred');
                } else {
                    $model->addErrors($modelErrors['modelErrors']);
                ...
                //error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO DELETE RNotifications  | '
                        . CJSON::encode($exc), __CLASS__, __FUNCTION__, __LINE__);
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 'sorryAnErrorOccurredWhileDeletingThe{model}Record', array('{model}' => 'Notifications')));
                $this->render('delete', array(
                    'model' => $model,
                ));
                Yii::app()->end();
    ...
    public function actionAdmin() {
        $model = new RNotifications('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['RNotifications']))
            $model->attributes = $_GET['RNotifications'];
    ...
    public function loadModel($id) {
        ...
        try {
            $model = RNotifications::model()->findById($id);
        } catch (Exception $exc) {
