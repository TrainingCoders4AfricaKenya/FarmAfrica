...
class ProductTypesController extends Controller
{
		...
		if(isset($_POST['RProductTypes'])){
			...
			try{
                ...
                if ($saveOK) {
                    ...
                    $this->redirect(array('view', 'id' => $model->productTypeID));
                } else {
				...
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON CREATE RProductTypes | CODE: '
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    ...
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO CREATE RProductTypes | '
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO CREATE RProductTypes  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileCreatingThe{model}Record', array('{model}' => 'ProductTypes')));
                $this->render('create', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['RProductTypes'])){
			...
			try{
				...
				//Catch EActiveResourceRequestException which is thrown in case of any errors
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON UPDATE RProductTypes | CODE: '
                        . $resourceExc->getCode() . ' | MESSAGE: ' . $resourceExc->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
                //an error occurred while doing processing
                $modelErrors = array();
                $errorMsg = $resourceExc->getMessage();
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    //error that occurred wasn't model-related
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO UPDATE RProductTypes | '
                            .CJSON::encode($errorMsg),__CLASS__, __FUNCTION__, __LINE__ );
                    Yii::app()->user->setFlash('error', 'Error occurred');
                }
                else{
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO UPDATE RProductTypes  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileUpdatingThe{model}Record', array('{model}' => 'ProductTypes')));
                $this->render('update', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	public function actionDelete($id)
	{
		...
		if(isset($_POST['RProductTypes'])){
			...
			try{
				...
				//Catch EActiveResourceRequestException which is thrown in case of any errors
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON DELETE RProductTypes | CODE: '
                        . $resourceExc->getCode() . ' | MESSAGE: ' . $resourceExc->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
                //an error occurred while doing processing
                $modelErrors = array();
                $errorMsg = $resourceExc->getMessage();
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    //error that occurred wasn't model-related
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO DELETE RProductTypes | '
                            .CJSON::encode($errorMsg),__CLASS__, __FUNCTION__, __LINE__ );
                    Yii::app()->user->setFlash('error', 'Error occurred');
                }
                else{
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO DELETE RProductTypes  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileDeletingThe{model}Record', array('{model}' => 'ProductTypes')));
                $this->render('delete', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	{
		$this->redirect(array('productTypes/admin'));
	}
	...
	{
		$model=new RProductTypes('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['RProductTypes']))
			$model->attributes=$_GET['RProductTypes'];
	...
	public function loadModel($id)
	{
		...
		try {
            $model = RProductTypes::model()->findById($id);
        } catch (Exception $exc) {
	 ...
	 * @param ProductTypes $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='product-types-form')
		{
