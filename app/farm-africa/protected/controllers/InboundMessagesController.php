...
class InboundMessagesController extends Controller
{
	/**
		...
		if(isset($_POST['RInboundMessages'])){
			...
			try{
                ...
                if ($saveOK) {
                    ...
                    $this->redirect(array('view', 'id' => $model->inboundMessageID));
                } else {
				...
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON CREATE RInboundMessages | CODE: '
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    ...
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO CREATE RInboundMessages | '
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO CREATE RInboundMessages  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileCreatingThe{model}Record', array('{model}' => 'InboundMessages')));
                $this->render('create', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['RInboundMessages'])){
			...
			try{
				...
				//Catch EActiveResourceRequestException which is thrown in case of any errors
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON UPDATE RInboundMessages | CODE: '
                        . $resourceExc->getCode() . ' | MESSAGE: ' . $resourceExc->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
                //an error occurred while doing processing
                $modelErrors = array();
                $errorMsg = $resourceExc->getMessage();
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    //error that occurred wasn't model-related
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO UPDATE RInboundMessages | '
                            .CJSON::encode($errorMsg),__CLASS__, __FUNCTION__, __LINE__ );
                    Yii::app()->user->setFlash('error', 'Error occurred');
                }
                else{
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO UPDATE RInboundMessages  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileUpdatingThe{model}Record', array('{model}' => 'InboundMessages')));
                $this->render('update', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	public function actionDelete($id)
	{
		...
		if(isset($_POST['RInboundMessages'])){
			...
			try{
				...
				//Catch EActiveResourceRequestException which is thrown in case of any errors
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON DELETE RInboundMessages | CODE: '
                        . $resourceExc->getCode() . ' | MESSAGE: ' . $resourceExc->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
                //an error occurred while doing processing
                $modelErrors = array();
                $errorMsg = $resourceExc->getMessage();
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    //error that occurred wasn't model-related
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO DELETE RInboundMessages | '
                            .CJSON::encode($errorMsg),__CLASS__, __FUNCTION__, __LINE__ );
                    Yii::app()->user->setFlash('error', 'Error occurred');
                }
                else{
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO DELETE RInboundMessages  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileDeletingThe{model}Record', array('{model}' => 'InboundMessages')));
                $this->render('delete', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	{
		$this->redirect(array('inboundMessages/admin'));
	}
	...
	{
		$model=new RInboundMessages('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['RInboundMessages']))
			$model->attributes=$_GET['RInboundMessages'];
	 ...
	 * @return InboundMessages the loaded model
	...
	public function loadModel($id)
	{
		...
		try {
            $model = RInboundMessages::model()->findById($id);
        } catch (Exception $exc) {
	 ...
	 * @param InboundMessages $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='inbound-messages-form')
		{
