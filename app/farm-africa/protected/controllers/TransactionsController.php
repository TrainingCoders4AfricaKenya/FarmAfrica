...
class TransactionsController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['RTransactions'])){
			...
			try{
				...
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON CREATE RTransactions | CODE: '
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    ...
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO CREATE RTransactions | '
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO CREATE RTransactions  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileCreatingThe{model}Record', array('{model}' => 'Transactions')));
                $this->render('create', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['RTransactions'])){
			...
			try{
				...
				//Catch EActiveResourceRequestException which is thrown in case of any errors
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON UPDATE RTransactions | CODE: '
                        . $resourceExc->getCode() . ' | MESSAGE: ' . $resourceExc->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
                //an error occurred while doing processing
                $modelErrors = array();
                $errorMsg = $resourceExc->getMessage();
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    //error that occurred wasn't model-related
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO UPDATE RTransactions | '
                            .CJSON::encode($errorMsg),__CLASS__, __FUNCTION__, __LINE__ );
                    Yii::app()->user->setFlash('error', 'Error occurred');
                }
                else{
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO UPDATE RTransactions  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileUpdatingThe{model}Record', array('{model}' => 'Transactions')));
                $this->render('update', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	public function actionDelete($id)
	{
		...
		if(isset($_POST['RTransactions'])){
			...
			try{
				...
				//Catch EActiveResourceRequestException which is thrown in case of any errors
				Utils::log('EXCEPTION', 'EActiveResourceRequestException ON DELETE RTransactions | CODE: '
                        . $resourceExc->getCode() . ' | MESSAGE: ' . $resourceExc->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
                //an error occurred while doing processing
                $modelErrors = array();
                $errorMsg = $resourceExc->getMessage();
                ...
                if($modelErrors == null || empty($modelErrors) || !isset($modelErrors['modelErrors'])){
                    //error that occurred wasn't model-related
                    Utils::log('ERROR', 'NON-MODEL ERROR OCCURRED WHILE TRYING TO DELETE RTransactions | '
                            .CJSON::encode($errorMsg),__CLASS__, __FUNCTION__, __LINE__ );
                    Yii::app()->user->setFlash('error', 'Error occurred');
                }
                else{
				...
				//error that occurred wasn't model-related
                Utils::log('EXCEPTION', 'AN Exception OCCURRED WHILE TRYING TO DELETE RTransactions  | '
                        .CJSON::encode($exc),__CLASS__, __FUNCTION__, __LINE__ );
                Yii::app()->user->setFlash('error', Yii::t(Yii::app()->language, 
                	'sorryAnErrorOccurredWhileDeletingThe{model}Record', array('{model}' => 'Transactions')));
                $this->render('delete', array(
                    'model' => $model,
                ));
                Yii::app()->end();
	...
	{
		$this->redirect(array('transactions/admin'));
	}
	...
	{
		$model=new RTransactions('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['RTransactions']))
			$model->attributes=$_GET['RTransactions'];
	...
	public function loadModel($id)
	{
		...
		try {
            $model = RTransactions::model()->findById($id);
        } catch (Exception $exc) {
