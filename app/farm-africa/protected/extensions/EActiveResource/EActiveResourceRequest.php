...
class EActiveResourceRequest {
    ...
    public function run() {
        ...
        //had to modify this because original wasn't working well with POST
        if ($this->getMethod() == self::METHOD_POST) {
            $this->setOption(CURLOPT_POSTFIELDS, http_build_query($this->getData()));
        } else {
...
//                $this->setOption(CURLOPT_HTTPHEADER,$this->getHeader());
    ...
    public function checkErrors() {
        ...
        if ($responseCode && $responseCode < 400)
            ...
            //modified this so that it only has the response without the "error part"
            $errorMessage = $this->_response;
